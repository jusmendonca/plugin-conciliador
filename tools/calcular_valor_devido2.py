...
def calcular_valor_devido_com_13(csv_path, data_inicial_parcelas, data_final_parcelas):
    ...
    Valor Proporcional da RM no Mês Inicial (atualizada): R$ {valor_proporcional_rm_inicial_formatado}
    
    Valor Integral das RM nos Meses Completos [{df_meio['dib'].min().strftime('%m/%Y') if not df_meio.empty else "N/A"} a {df_meio['dib'].max().strftime('%m/%Y') if not df_meio.empty else "N/A"}] (atualizada): R$ {valor_integral_rm_completos_formatado}
    
    Dias do Mês Final das Parcelas [{mes_final:02}/{ano_final}]: {dias_no_mes_final} dias
    Valor Proporcional da RM no Mês Final (atualizado): R$ {valor_proporcional_rm_final_formatado}
    
    Cálculo da Gratificação Natalina:    
    Gratificação Natalina Proporcional para o Ano Inicial [{ano_inicial}]: R$ {gratificacao_natalina_proporcional_inicial_formatado}
    Gratificação Natalina Integral para Anos Intermediários [{', '.join(map(str, anos_completos)) if anos_completos else 'N/A'}]: R$ {gratificacao_natalina_integral_formatado}
    Gratificação Natalina Proporcional para o Ano Final [{ano_final}]: R$ {gratificacao_natalina_proporcional_final_formatado}
    
    Valor Total Devido: R$ {valor_total_devido_formatado}
    ---------------------------
    """
...
data_inicial_parcelas = '02/09/2021'
